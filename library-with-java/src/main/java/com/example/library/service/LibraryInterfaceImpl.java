...
public class LibraryInterfaceImpl implements LibraryInterface {
    private static final Logger log = Logger.getLogger(LibraryInterfaceImpl.class.getName());
    private final BookDataAccess dataAccess;

    public LibraryInterfaceImpl(BookDataAccess dataAccess) {
    ...
    @Override
    public RentedBook findBookWithSubject(String subject) {
    ...
    public RentedBook rentBook(String isbn) {
        ...
        return dataAccess.findBookByIsbn(isbn)
                .orElseThrow(() -> new RuntimeException("Book is already rented"));
    }
